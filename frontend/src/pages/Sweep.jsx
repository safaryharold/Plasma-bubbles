...
export default function Sweep() {
  ...
  const { job, viz, err, run, saveExperiment } = useIbpJob();
  ...
  const save = async () => {
    const res = await saveExperiment(form, form.name);
    if (res.ok) { setSaveMsg("Experiment saved ✓"); setTimeout(() => setSaveMsg(null), 3000); }
  };
  ...
  return (
    <div className="space-y-6 animate-fade-in" data-testid="sweep-page">
      <SweepHeader cells={cells} />
      <div className="grid lg:grid-cols-[1fr_1.4fr] gap-6">
        <SweepForm form={form} setForm={setForm} cells={cells}
          err={err} saveMsg={saveMsg}
          onSubmit={() => run(form)} onSave={save} />
        <SweepResult job={job} viz={viz} onDownload={download} />
      </div>
    </div>
  );
}

function SweepHeader({ cells }) {
  return (
...
function SweepForm({ form, setForm, cells, err, saveMsg, onSubmit, onSave }) {
  return (
    <div className="border border-[#2A2D35] p-6 space-y-5" data-testid="sweep-form">
      ...
      <AxisFieldset title="longitude axis (°)" form={form} setForm={setForm} ks={["lon_min", "lon_max", "lon_step"]}
        mins={[-180, -180, 1]} maxs={[180, 180, 180]} step={1} tidPrefix="sweep-lon" />
      <AxisFieldset title="local time axis (h)" form={form} setForm={setForm} ks={["lt_min", "lt_max", "lt_step"]}
        mins={[0, 0, 0.1]} maxs={[24, 24, 12]} step={0.1} tidPrefix="sweep-lt" />
      ...
      <div className="flex gap-3">
        <button onClick={onSubmit} disabled={cells > 10000 || cells === 0} data-testid="run-sweep-btn"
        ...
        <button onClick={onSave} data-testid="save-experiment-btn"
...
function AxisFieldset({ title, form, setForm, ks, mins, maxs, step, tidPrefix }) {
  return (
    <div className="border-t border-[#2A2D35] pt-4">
      <div className="mono text-[10px] uppercase tracking-[0.25em] text-[#565D6D] mb-3">— {title}</div>
      <div className="grid grid-cols-3 gap-3">
        {["min", "max", "step"].map((label, i) => (
          <Num key={ks[i]} label={label} v={form[ks[i]]} min={mins[i]} max={maxs[i]} step={step}
            k={ks[i]} form={form} setForm={setForm} tid={`${tidPrefix}-${label}`} />
        ))}
      </div>
    </div>
  );
...
function SweepResult({ job, viz, onDownload }) {
  return (
    <div className="border border-[#2A2D35] bg-[#121418] min-h-[600px] flex flex-col" data-testid="sweep-result">
      ...
      {!job ? (
        ...
        <CompletedResult job={job} viz={viz} onDownload={onDownload} />
      ) : null}
    </div>
  );
}
...
function CompletedResult({ job, viz, onDownload }) {
  return (
    <div className="flex-1 flex flex-col">
      ...
      <div className="p-4 border-t border-[#2A2D35] flex gap-3 flex-wrap">
        <button onClick={() => onDownload("csv")} data-testid="download-csv-btn"
        ...
        <button onClick={() => onDownload("netcdf")} data-testid="download-nc-btn"
        ...
        <button onClick={() => onDownload("parquet")} data-testid="download-parquet-btn"
          className="h-10 px-4 border border-[#2A2D35] hover:border-[#0047FF] hover:text-[#0047FF] text-white mono text-xs uppercase tracking-[0.25em] flex items-center gap-2 transition-colors">
